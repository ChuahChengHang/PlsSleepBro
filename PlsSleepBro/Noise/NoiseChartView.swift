    ...
    var body: some View {
        VStack {
            if !noiseData.isEmpty {
                ...
                let hourlyData = (startHour..<endHour).map { hour -> (time: Date, value: Double) in
...
#Preview {
    NoiseChartView(date: .constant(Date.now), offSet: .constant(0), suggestions: .constant([]), selectedDateDataEmpty: .constant(false))
}
